...
class Day11(dataFile: DataFile) {
    private val monkeys = fileToString(2022, 11, dataFile)
        .split("\n\n")
        .associate { line ->
            ...
            val items = lines[1]
                ...
                .toMutableList()
            val operation = lines[2].split(" ").drop(5).let {
                val lhs = it[0]
                val rhs = it[2]
            ...
            val test: (BigInteger) -> BigInteger = lines[3].split(" ").drop(3).last().toInt().toBigInteger().let {
    ...
    private val lcm = fileToString(2022, 11, dataFile)
        .split("\n\n")
        .map { block ->
    ...
    private fun trackMonkeys(rounds: Int, worryFunc: (BigInteger) -> BigInteger): List<BigInteger> {
