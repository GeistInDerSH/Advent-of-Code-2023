...
class Day25(dataFile: DataFile) {
    ...
    private fun decode(line: String) = line
        .map { decodeLookupTable[it]!! }
        .reduce { acc, i -> acc * 5 + i }

    private fun encode(n: Long) = generateSequence(n) { (it + 2) / 5 } // 0 to 5
        .takeWhile { it != 0L } // Take until we get a min value
        .joinToString("") { val idx = ((it + 2) % 5) - 2; encodeLookupTable[idx]!! } // -2 to 2
        .reversed()
