...
class Day8(dataFile: DataFile) {
    private val instructions = fileToStream(2020, 8, dataFile)
    ...
    private fun List<Instruction>.execute(): Pair<Int, Boolean> {
        ...
        while (rip !in indicesSeen && rip < this.size) {
            indicesSeen.add(rip)
            rip += when (val instruction = this[rip]) {
                is Instruction.Acc -> {
    ...
    private fun List<Instruction>.executeWithInvert() = sequence {
        for (i in this@executeWithInvert.indices) {
            ...
            lst[i] = when (val instruction = lst[i]) {
