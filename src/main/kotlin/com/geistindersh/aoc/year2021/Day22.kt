...
class Day22(dataFile: DataFile) {
	private val cubes = fileToStream(2021, 22, dataFile)
		.map { line ->
	...
	private data class RebootCube(val isOn: Boolean, val x: IntRange, val y: IntRange, val z: IntRange) {
		fun volume() =
			(if (isOn) 1 else -1) * x.size() * y.size() * z.size()

		fun hasOverlap(other: RebootCube) =
			x.hasOverlap(other.x) && y.hasOverlap(other.y) && z.hasOverlap(other.z)

		fun intersection(other: RebootCube) = RebootCube(
			!isOn,
	...
	fun part1() = RebootCube(true, -50..50, -50..50, -50..50).let { cube ->
