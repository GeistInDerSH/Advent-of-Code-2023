...
class Day25(dataFile: DataFile) {
	private val valley = fileToStream(2021, 25, dataFile)
		.flatMapIndexed { row, s ->
			s.mapIndexed { col, c ->
				when (c) {
					'>' -> Point(row, col) to Direction.East
					'v' -> Point(row, col) to Direction.South
					else -> Point(row, col) to null
				}
			}
		}
		.toMap()
	...
	private data class Valley(val valley: Map<Point, Direction?>, val moved: Int = -1) {
		...
		fun next(): Valley {
			...
			for (row in 0..rowMax) {
				val rowStart = newMap[Point(row, 0)]
				var col = 0
				while (col <= colMax) {
			...
			for (col in 0..colMax) {
				val colStart = newMap[Point(0, col)]
				var row = 0
				while (row <= rowMax) {
					val point = Point(row, col)
					row += 1
