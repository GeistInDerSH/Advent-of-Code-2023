...
class Day10(dataFile: DataFile) {
	private val chunks = fileToStream(2021, 10, dataFile)
		.map { it.toCharArray() }
		.toList()
	private val errorScoreTable = mapOf(
		')' to 3,
		']' to 57,
		'}' to 1197,
		'>' to 25137,
	)
	private val autocompleteScoreTable = mapOf(
		')' to 1L,
		']' to 2L,
		'}' to 3L,
		'>' to 4L,
	...
	private fun completeSequence(chunk: CharArray): String? {
		...
		while (stack.isNotEmpty()) {
			val char = when (val top = stack.pop()) {
				'(' -> ')'
				'[' -> ']'
				'{' -> '}'
				'<' -> '>'
