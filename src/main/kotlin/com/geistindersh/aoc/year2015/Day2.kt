...
class Day2(dataFile: DataFile) {
    private val data = fileToStream(2015, 2, dataFile)
        .map { line ->
            "[0-9]+".toRegex().findAll(line).map { it.value }.map(String::toInt).toList().sorted()
        }
        .toList()

    private fun surfaceArea(l: Int, h: Int, w: Int) = 2 * l * w + 2 * w * h + 2 * h * l
    private fun areaOfSmallestSide(l: Int, h: Int, w: Int) = (l * h * w) / l.coerceAtLeast(h).coerceAtLeast(w)
    ...
    private fun ribbonLength(l: Int, h: Int, w: Int) = l * h * w
