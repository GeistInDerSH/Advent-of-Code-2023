...
class Day25(dataFile: DataFile) {
	...
	private data class Code(val point: Point, val code: Long) {
		fun next(): Code {
			val (row, col) = point
			val newPoint = if (row - 1 < 1) Point(col + 1, 1) else Point(row - 1, col + 1)
			val newCode = (code * 252533) % 33554393
			return Code(newPoint, newCode)
		}
	}

	fun part1() = generateSequence(Code(Point(1, 1), 20151125)) { it.next() }
